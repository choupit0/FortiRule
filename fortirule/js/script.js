...
function handleSubmit(event) {
  ...
  if (['Authorize_IP', 'Block_IP'].includes(action)) {
	...
	if (duplicateIPs.length > 0) {
	  showMessage(`<b>The following IP addresses are duplicated</b>: ${duplicateIPs.join(', ')}`);
	  return;
	}
	...
	for (let i = 0; i < ipLines.length; i++) {
	  ...
	  if (!isValidIP(ipLine)) {
		showMessage(`<b>The IP address format on line ${i + 1} is invalid</b>. \n<b>IPv4</b>: Enter as e.g. 192.168.2.100, 172.31.0.0/16, or 172.16.8.1-172.16.8.100 \n<b>IPv6</b>: Enter as e.g. 2001:0db8::eade:27ff:fe04:9a01, 2001:0db8::eade:27ff:fe04:9a01/120 or 2001:0db8::eade:27ff:fe04:aa01-2001:0db8::eade:27ff:fe04:ab01`);
		return;
	  }
	}
	  ...
	  if (duplicateDomains.length > 0) {
		showMessage(`<b>The following domains/URLs are duplicated</b>: ${duplicateDomains.join(', ')}`);
		return;
	  }
  ...
  if (['Authorize_domain', 'Block_domain'].includes(action)) {
    ...
    if (invalidDomains.length > 0) {
      showMessage(`<b>The following domains/URLs are not valid</b>: ${invalidDomains.join(', ')}`);
      return;
    }
  }
